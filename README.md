...
- 手掌左右移动控制 3D 卡牌转盘
- 鼠标或手指轻划即可让牌流惯性滑动并自动减速吸附，轻点当前牌选中
- 宽幅无限牌流视觉：卡牌保持间距并自然延伸至屏幕两侧之外
- 握拳抓取当前卡牌
- 🖐️ 左右移动手掌切换 1 / 3 / 5 张牌阵
- 无摄像头时支持按钮抽牌
- MediaPipe 浏览器本地手势识别，摄像头画面不上传
- 完整 78 张塔罗数据与正逆位基础牌义
- DeepSeek AI 根据问题、牌阵位置和牌面关系生成个性化解读
- DeepSeek 会根据小樱风／黑暗风切换叙事人格，并用故事方式串联整组牌
- AI 解读以直接结论和现实建议为主，氛围描写不超过两成；感情问题会先给出明确的牌面倾向，再提供可观察的现实信号
- AI 请求时先滚动到加载区，答案返回后会再次平滑对齐到解读区顶部
- 每张牌会引用一个具体的经典画面元素并解释其含义；输出会清理破折号
- 👍 手势完成到 100% 后会立即平滑滚动到 AI 解读区
- 每轮初始解读随机附带一个彩蛋：守护词、幸运意象、放下的话、七日练习或牌面反问
- “输入问题 → 选择牌阵 → 牌组展开 → 抽牌 → AI 解读”的分镜式引导流程
- 78 张经典 Rider–Waite 牌面图片与 Sakura 粉色卡框
- 揭晓后松开手掌不会再清空结果，AI 按钮会保持显示
- 首次点击允许摄像头后，支持全程手势控制
- 初次 AI 解读后支持最多 3 次按钮追问
- AI 解读完成后可将本轮牌意生成一杯动态主题特饮，包含变化的杯型、液体色彩、杯饰、酒精版与无酒精版配方
- 右上角“重新抽卡”在所有步骤常驻
...
- 牌阵步骤：🖐️ 左右移动手掌选择 1 / 3 / 5 张牌阵，✊ 握拳确认
- 牌组展开：握拳确认牌阵后自动完成，约 1 秒即可抽牌
- 牌阵确认后有 1.4 秒手势过渡锁，并要求先松拳再重新握拳，避免连续误触
- 抽牌步骤：🖐 左右移动卡牌，✊ 握拳抓取
- 结果步骤：👍 点赞请求 DeepSeek AI 深度解读
- 解读结果：🖐️ 上下移动手掌滚动页面，☝️ 单指向上生成牌意特调
- 重新开始：✌ 保持胜利手势

追问只使用页面按钮提交，不占用任何手势；每轮最多追问 3 次。

页面底部会随流程显示当前可用手势及确认进度。
页面最下方同时常驻完整手势图例，包括 👍 请求 AI 深度解读和追问按钮说明，未开启摄像头时也能看到。
